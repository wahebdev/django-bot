...
class TelegramUser(BaseModel):
    ...
    user_id = models.BigIntegerField('user_id', unique=True)
    ...
    block = models.BooleanField('block', default=False)
    admin = models.BooleanField('admin', default=False)
    usage = models.BigIntegerField('usage', default=0)

    def __str__(self):
        """STR Representation"""
        return f"{self.user_id} - @{self.username} - {self.first_name}  {self.last_name} {self.admin} {self.block} {self.usage}"
...
class TelegramUserHistory(BaseModel):
    ...
    telegram_user = models.ForeignKey(TelegramUser, on_delete=models.CASCADE, verbose_name='Telegram user', null=True,
                                      blank=True)
